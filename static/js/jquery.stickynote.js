jQuery(document).ready(function ($) {
    ...
    $.CoaStickyNote = function(element, options) {
        ...
        var defaults = {
            ...
            noteDimension: { width: "235px", height: "100px" },
        ...
        var setMissingOptions = function(note) {
			...
			// User story id is not set
            if (!valueSet(note.settings.idUserStory)) {
                note.settings.idUserStory = plugin.settings.idUserStory;
            }
        };
	...
	$("#init").click(function () {		
		for (i = 0; i < notes.length; i++) {
			plugin.settings = $.extend(true, {}, defaults, notes[i]);	
			createNoteBox($element);
		}						
    });			
		
	$("#new").click(function () {		
			plugin.settings = $.extend(true, {}, defaults, mynote);	
			createNoteBox($element);							
    });	
